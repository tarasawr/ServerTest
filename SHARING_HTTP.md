# Sharing HTTP API (`projects.js`)

Краткое описание REST-эндпоинтов реестра шаринга проектов. Реализация: [`projects.js`](./projects.js), подключение из [`server.js`](./server.js) через `handleRequest`.

## Общее

- **Базовый URL**: тот же хост и порт, что у HTTP-сервера (по умолчанию `http://localhost:3000`).
- Заголовок для тел JSON: `Content-Type: application/json`.
- Ошибки: JSON `{ "error": "message" }` с соответствующим HTTP-статусом.
- Данные хранятся **в памяти** процесса Node (после перезапуска сервера очищаются).

## Модель ролей

Каждый проект имеет **глобальную роль** (`globalRole`) — настройку по умолчанию для всех участников.  
Каждый участник может иметь **индивидуальную роль** (или не иметь — `null`).  
**Эффективная роль** = индивидуальная роль (если задана) ИЛИ `globalRole` проекта.

Все эндпоинты возвращают уже **вычисленную эффективную роль** — фронтенд не получает `null`.

| Значение роли | Описание |
|---------------|----------|
| `owner` | Владелец (создатель проекта) |
| `can_edit` | Может вносить изменения |
| `can_view` | Только просмотр (умолчание `globalRole`) |

---

## Эндпоинты

### `POST /projects`

Регистрация проекта в реестре. Повтор с тем же `projectId` обновляет XML (если передан), дату синхронизации и возвращает успех — **идемпотентно**.

**Тело:**

| Поле | Обязательно | Описание |
|------|-------------|----------|
| `projectId` | да | Строковый идентификатор проекта |
| `ownerUserId` | да | ID владельца |
| `ownerName` | нет | Имя владельца (по умолчанию `Unknown`) |
| `projectXml` | нет | XML снапшот проекта |

**Успех (200):** `{ "ok": true, "projectId": "...", "shareUrl": "/projects/{projectId}" }`  
`shareUrl` — **относительный** путь; полная ссылка = база + `shareUrl`.  
Новый проект создаётся с `globalRole: "can_view"`.

**Ошибки:** `400` — нет `projectId` или `ownerUserId`.

---

### `GET /projects/user/:uid`

Список проектов, в которых участвует пользователь `:uid` (URL-декодирование применяется к сегменту).

**Успех (200):** `{ "projects": [ { "projectId", "ownerName", "role", "lastSyncDate" }, ... ] }`  
`role` — эффективная роль пользователя в каждом проекте.

---

### `POST /projects/:id/join`

Добавить пользователя в проект. Новый участник добавляется **без индивидуальной роли** (наследует `globalRole`). Уже существующий пользователь получает свою текущую эффективную роль.

**Тело:**

| Поле | Обязательно | Описание |
|------|-------------|----------|
| `userId` | да | ID пользователя |
| `name` | нет | Отображаемое имя |
| `avatarUrl` | нет | URL аватара |
| `isGuest` | нет | boolean |

**Успех (200):**

- новый участник: `{ "ok": true, "role": "<globalRole проекта>" }`
- уже в проекте: `{ "ok": true, "role": "<эффективная роль>", "alreadyMember": true }`

**Ошибки:** `404` — проект не найден; `400` — нет `userId`.

---

### `GET /projects/:id`

Метаданные проекта (без полного `projectXml` в ответе).

**Успех (200):** `{ "projectId", "ownerUserId", "ownerName", "lastSyncDate", "globalRole", "userCount" }`

**Ошибки:** `404` — проект не найден.

---

### `PUT /projects/:id/globalRole`

Изменить глобальную роль проекта (настройка по умолчанию для всех участников без индивидуальной роли). **Только владелец.**

**Тело:** `{ "globalRole": "can_view" | "can_edit", "ownerUserId": "<id владельца>" }`

**Успех (200):** `{ "ok": true, "globalRole": "..." }`

**Ошибки:** `400` — неверная роль; `403` — не владелец; `404` — проект не найден.

---

### `GET /projects/:id/users`

Список всех участников.

**Успех (200):**
```json
{
  "globalRole": "can_view",
  "users": [ { "userId", "name", "avatarUrl", "role", "isGuest" }, ... ]
}
```
`role` — эффективная роль каждого участника (индивидуальная или `globalRole`). Никогда не `null`.

**Ошибки:** `404` — проект не найден.

---

### `GET /projects/:id/users/:uid/role`

Эффективная роль конкретного участника (`:uid` URL-декодируется).

**Успех (200):** `{ "userId", "role" }`  
Роли: `owner`, `can_edit`, `can_view`.

**Ошибки:** `404` — проект не найден или пользователь не в проекте.

---

### `PUT /projects/:id/users/:uid/role`

Установить **индивидуальную** роль участника. Нельзя изменить роль владельца. Только `can_view` или `can_edit`.

**Тело:** `{ "role": "can_view" | "can_edit" }`

**Успех (200):** `{ "ok": true, "role": "..." }`

**Ошибки:** `400` — неверная роль или попытка изменить роль владельца; `404` — проект или пользователь не найден.

---

### `DELETE /projects/:id/users`

Удалить **всех** участников (кроме владельца) из проекта. **Только владелец.**

**Тело:** `{ "ownerUserId": "<id владельца>" }` — должен совпадать с `ownerUserId` проекта.

**Успех (200):** `{ "ok": true, "removedCount": N }` — `removedCount` — количество удалённых участников (0, если никого, кроме владельца, не было).

**Ошибки:** `403` — не владелец; `404` — проект не найден.

---

### `DELETE /projects/:id/users/:uid`

Удалить конкретного участника из проекта. **Только владелец.**

**Тело:** `{ "ownerUserId": "<id владельца>" }` — должен совпадать с `ownerUserId` проекта.

**Успех (200):** `{ "ok": true }`

**Ошибки:** `403` — не владелец; `400` — попытка удалить владельца; `404` — проект/пользователь не найден.

---

### `PUT /projects/:id/sync`

Обновить `lastSyncDate` у проекта (тело может быть пустым `{}`).

**Успех (200):** `{ "ok": true, "lastSyncDate": "<ISO8601>" }`

**Ошибки:** `404` — проект не найден.

---

### `DELETE /projects/:id`

Удалить проект из реестра целиком. **Только владелец.**

**Тело:** `{ "ownerUserId": "<id владельца>" }` — должен совпадать с `ownerUserId` проекта.

**Успех (200):** `{ "ok": true }`

**Ошибки:** `403` — не владелец; `404` — проект не найден.

**Побочный эффект:** все активные WebSocket-сессии, привязанные к этому проекту (поле `session.projectId`), **принудительно завершаются**: участникам отправляется сообщение `{ type: "session_closed", reason: "sharing_stopped" }`, после чего их соединения закрываются.

---

### `GET /projects/:id/session`

Найти **активную** WebSocket-сессию, привязанную к проекту. Возвращает первую сессию с `projectId === id` и хотя бы одним игроком.

**Успех (200):** `{ "inviteCode": "..." }`

**Ошибки:** `404` — проект не найден или нет активной привязанной сессии.

> Используется клиентом сразу после `POST /projects/:id/join`, чтобы получить `inviteCode` и автоматически подключиться к WebSocket-сессии.

---

### `PUT /sessions/:inviteCode/project`

Привязать **активную** WebSocket-сессию к зарегистрированному проекту. `inviteCode` — код приглашения сессии на этом же сервере.

**Тело:** `{ "projectId": "..." }`

**Успех (200):** `{ "ok": true }` — у сессии выставляется `projectId`; дальнейшие обновления XML через `update_state` могут синхронизироваться в реестр через `onXmlUpdated`.

**Ошибки:** `400` — нет `projectId`; `404` — проект не найден **или** сессии с таким `inviteCode` нет (частый случай при опечатке или другом инстансе сервера).

---

## Утилитарные эндпоинты (`server.js`)

Реализованы напрямую в `server.js`, не через `projects.js`.

### `GET /sessions`

Список всех активных WebSocket-сессий (legacy-сессия `__legacy__` исключена).

**Успех (200):**
```json
{ "sessions": [ { "id", "inviteCode", "players", "linkPermission" }, ... ] }
```
`players` — текущее число подключённых участников.

---

### `GET /sessions/latest/project`

XML проекта из первой активной сессии (с хотя бы одним игроком).

**Успех (200):** `{ "inviteCode": "...", "projectXml": "..." }`

**Ошибки:** `404` — нет активных сессий с игроками.

---

### `GET /bots`

Показать текущие настройки ботов.

**Успех (200):** `{ "botCount": N, "viewMode": "random" | "2d" | "3d" }`

**Query-параметры (опционально):**

| Параметр | Описание |
|----------|----------|
| `count=N` | Установить число ботов на сессию (0–10). При `N > 0` боты немедленно спауняться во всех активных сессиях. |
| `mode=2d\|3d\|random` | Принудительный режим просмотра для ботов. |

---

### `GET /rooms`

Разобранный список комнат из XML первой активной сессии (для отладки).

**Успех (200):**
```json
{ "rooms": [ { "index", "center": { "x", "z" }, "area", "indoor" }, ... ] }
```
При отсутствии активной сессии возвращает `{ "rooms": [], "error": "No active session" }` со статусом `200`.

---

### `GET /test` / `POST /test`

Координация тестов между несколькими экземплярами Unity.

- **GET** — вернуть текущее состояние: `{ "phase": "idle" | "...", ... }`  
- **POST** — перезаписать состояние произвольным JSON-объектом; возвращает `{ "ok": true }`.

---

## Связь с Unity

Клиент Unity использует константу `MultiplayerConnection.HttpUrl` и класс `MultiplayerSharingApi`:

| Метод C# | Эндпоинт |
|----------|----------|
| `RegisterProject` | `POST /projects` |
| `JoinProject` | `POST /projects/{id}/join` |
| `GetUserRole` | `GET /projects/{id}/users/{uid}/role` |
| `GetProjectGlobalRole` | `GET /projects/{id}` |
| `SetProjectGlobalRole` | `PUT /projects/{id}/globalRole` |
| `SetUserRole` | `PUT /projects/{id}/users/{uid}/role` |
| `RemoveUser` | `DELETE /projects/{id}/users/{uid}` |
| `RemoveAllUsers` | `DELETE /projects/{id}/users` |
| `DeleteProject` | `DELETE /projects/{id}` |
| `GetProjectUsers` | `GET /projects/{id}/users` |
| `GetProjectSessionInviteCode` | `GET /projects/{id}/session` |
| `LinkSessionToProject` | `PUT /sessions/{invite}/project` |
